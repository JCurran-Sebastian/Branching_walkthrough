...
class Branching_Process:
    ...
    def pgf_immigration(self, s, immigration, count = 'prev'):
        ...
        immigration_tau = immigration(D_tau) 
    ...
    def analytic_mean(self, immigration = False):
        ...
        def lam(t, tau):
            return (transmission_rate(t + tau) * 
                    (infectiousness_profile(t) - infectiousness_profile(t-time_step)) * 
                    (1 - lifetime_distribution(t, tau)))
        ...
        for n in range(N + 1):
            for i in range(n + 1):
                if i == 0:
                    ...
                    if immigration == True:
                        eta = self.immigration
                        fhat[n, i] = (
                                   np.sum(fhat[n, i - np.arange(1, i + 1)] * 
                                lam(time_step * np.arange(1, i + 1), (n - i) * time_step) + eta(time_step * np.arange(1, i + 1))[0]  ) 
                                )
                    else:
               
                        fhat[n, i] = (h(n * time_step, (n - i) * time_step) +
                                    np.sum(fhat[n, i - np.arange(1, i + 1)] * 
                                    lam(time_step * np.arange(1, i + 1), (n - i) * time_step) ) )  
